...
# Podmienený výber riadkov z dátového rámca, kde hodnota v type_list je "A"
riadky_typu_A = data[data['type'] == 'A']

print("Riadky s hodnotou 'A' v type_list:")
print(riadky_typu_A)
...
print("==================")
print("PARAMETRE")
away_goals60_list = [None] * pocet_riadkov
home_goals60_list = [None] * pocet_riadkov
for i in range(pocet_riadkov):
    if (outcome_list[i]== "home win REG"):
        home_goals60_list[i] = home_goals_list[i]
        away_goals60_list[i] = away_goals_list[i]
    if (outcome_list[i]== "home win OT"):
        home_goals60_list[i] = home_goals_list[i] - 1
        away_goals60_list[i] = away_goals_list[i]
    if (outcome_list[i]== "away win OT"):
        home_goals60_list[i] = home_goals_list[i]
        away_goals60_list[i] = away_goals_list[i] - 1
    if (outcome_list[i]== "away win REG"):
        home_goals60_list[i] = home_goals_list[i]
        away_goals60_list[i] = away_goals_list[i]
print("domáce góly v 60:", sum(home_goals60_list))
print("hosťujúce góly v 60:", sum(away_goals60_list))
goly60 = sum(home_goals60_list) + sum(away_goals60_list)
print("celkovo gólov v 60:", goly60)
r = goly60/pocet_riadkov
print("celkovo gólov na 60 minút:", r)
print("1/r = gol padne v priemere kazdych", 60/r, " minut")
print("11 minut a ", (60/r - 11)*60, "sekund")

#priemerny cas v sekundach 599.28340918494 (z druheho suboru)
print("1/r = ", 3600/r)
print("mean = ", 599.28340918494)
w = 3600/r - 599.28340918494
print("w = ",w)


def survival(t, w):
    return np.exp(-r*t + r*w*(1-np.exp(-t/w)))

def density(t, w):
    return r*(1-np.exp(-t/w))*survival(t, w)
