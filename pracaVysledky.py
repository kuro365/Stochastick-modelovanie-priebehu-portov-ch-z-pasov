...
l = 0
for i in range(1230):
    t1 = teamMatchPTS_list[i] + teamPTS5_list[i] + teamPTS6_list[i] + teamPTS7_list[i] + teamPTS8_list[i]
    t2 = opptMatchPTS_list[i] + opptPTS5_list[i] + opptPTS6_list[i] + opptPTS7_list[i] + opptPTS8_list[i]
    if (t1 > t2):
        w = w + 1
    if (t2 > t1):
        l = l + 1
...
# skutocne vsetko domace zapasy?
k = 0
for i in range(1230):
    if (teamLoc_list[i]=="Home"):
        k = k + 1
    else:
        print("hladane", i)
        print(teamLoc_list[i])
print(k)  

# tolko percent zapasov by mali vyhrat domaci
print(norm.cdf(0.153846, 0, 1), "% zapasov by mali vyhrat domaci")
print("--------------------")
...
plt.title('Q-Q graf bodových rozdielov v 1. štvrtine - NBA', fontsize=25)
...
l_8 = -7
l_9 = 7
# mi = 6.880636604774537
#
drift = 6.880636604774537
sigma2 = 26.525198938992045 ** 2
# t = 0
a_1 = (l_4 + ((1-t_1)*drift))/math.sqrt((1-t_1)*sigma2)
# t = 1/4
a_2 = (l_1 + ((1-t_2)*drift))/math.sqrt((1-t_2)*sigma2)
